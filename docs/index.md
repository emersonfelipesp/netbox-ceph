# netbox-ceph

`netbox-ceph` is a sibling NetBox plugin for
[`netbox-proxbox`](https://github.com/emersonfelipesp/netbox-proxbox).

Version 0.0.1.post2 is intentionally read-only. It mirrors Proxmox-managed Ceph
inventory through [`proxbox-api`](https://github.com/emersonfelipesp/proxbox-api)
and reuses `netbox-proxbox` backend context, branch lifecycle, endpoint
relationships, and job conventions.

## Compatibility

| NetBox | netbox-ceph | netbox-proxbox | Python |
| --- | --- | --- | --- |
| v4.5.8 | v0.0.1.post2 | >=0.0.25.post1,<0.1.0 | 3.12+ |
| v4.5.9 | v0.0.1.post2 | >=0.0.25.post1,<0.1.0 | 3.12+ |
| v4.6.0 | v0.0.1.post2 | >=0.0.25.post1,<0.1.0 | 3.12+ |
| v4.6.1 | v0.0.1.post2 | >=0.0.25.post1,<0.1.0 | 3.12+ |
| v4.6.2 | v0.0.1.post2 | >=0.0.25.post1,<0.1.0 | 3.12+ |
| v4.6.3 | v0.0.1.post2 | >=0.0.25.post1,<0.1.0 | 3.12+ |
| v4.6.4 | v0.0.1.post2 | >=0.0.25.post1,<0.1.0 | 3.12+ |
| v4.7.0 | v0.0.1.post2 | >=0.0.25.post1,<0.1.0 | 3.12+ |

## Out of scope for v0.0.1.post2

- Direct Ceph Dashboard API integration
- Prometheus metric ingestion
- External non-Proxmox Ceph clusters
- NetBox-to-Ceph write operations for reflected inventory (RGW/S3 and RBD
  objects are read-only reflected inventory in v1 — see [Models](models.md)
  for the full `CephRGW*`/`CephRBD*` model list)
