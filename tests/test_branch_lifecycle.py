"""Behavior tests for the Ceph branch-isolation decision wrapper."""
...
MODULE_PATH = (
    Path(__file__).resolve().parents[1] / "netbox_ceph" / "services" / "branch_lifecycle.py"
)


def _load_module(monkeypatch: pytest.MonkeyPatch, settings: object):
    """Load the wrapper with a controllable Ceph settings model."""
    ...
    setattr(models, "CephPluginSettings", CephPluginSettings)
    monkeypatch.setitem(sys.modules, "netbox_ceph.models", models)

    module_name = "netbox_ceph.services.branch_lifecycle_contract_test"
...
def _enabled_settings() -> SimpleNamespace:
    return SimpleNamespace(
        ...
        branch_name_prefix="nightly-ceph",
...
def test_enabled_and_available_branching_returns_ceph_settings(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    module = _load_module(monkeypatch, _enabled_settings())
    lifecycle = _typed_lifecycle("enabled")
    lifecycle.is_branching_available = lambda: pytest.fail(
        "the typed enabled decision must be preferred"
    )
...
def test_typed_decision_failure_does_not_fall_back_to_direct_writes(
    ...
    lifecycle = SimpleNamespace(
        ...
        is_branching_available=lambda: pytest.fail(
            "a broken typed resolver must not fall back to the legacy check"
        ),
    )
    monkeypatch.setattr(module, "_proxbox_branch_lifecycle", lambda: lifecycle)
...
def test_unreadable_settings_row_fails_before_runtime_resolution(
    ...
    monkeypatch.setattr(module, "CephPluginSettings", UnreadableSettings)
    monkeypatch.setattr(
        module,
        "_proxbox_branch_lifecycle",
...
def test_typed_disabled_decision_checks_runtime_for_ceph_enabled_setting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    module = _load_module(monkeypatch, _enabled_settings())
...
@pytest.mark.parametrize(
    ...
    [
        (True, "Branch merged."),
        (True, "Branch merged.", None),
    ],
)
def test_merge_wrapper_accepts_two_and_three_value_results(
    monkeypatch: pytest.MonkeyPatch,
    merge_result: tuple[object, ...],
) -> None:
    module = _load_module(monkeypatch, _enabled_settings())
    lifecycle = SimpleNamespace(merge_branch=lambda **_kwargs: merge_result)
    monkeypatch.setattr(module, "_proxbox_branch_lifecycle", lambda: lifecycle)
...
def test_published_0_0_26_post7_helper_surface_end_to_end(
    ...
    """Exercise the published helper shape admitted by the dependency floor."""
